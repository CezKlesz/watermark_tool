...
def process_image(
    ...
    if w >= h:
        nw = target_long_side
        nh = max(1, round(h * target_long_side / w))
    else:
        nh = target_long_side
        nw = max(1, round(w * target_long_side / h))
...
class WatermarkApp:
    ...
    def _build_ui(self) -> None:
        ...
        self.process_btn = ttk.Button(
            bf, text="▶  Przetwórz zdjęcia", command=self.start_processing
        )
    ...
    def start_processing(self) -> None:
        ...
        threading.Thread(
            target=self._worker, args=(src, dst, target, params), daemon=True
        ).start()

    def _worker(self, src: str, dst: str, target: int, params: dict) -> None:
        files = [
        ...
        if not files:
            self.root.after(0, lambda: (
                messagebox.showinfo(
                    ...
                    "Obsługiwane formaty: JPG, JPEG, PNG, TIF, TIFF, BMP, WEBP",
                ),
                self._reset(),
            ))
            return
        ...
        for i, fname in enumerate(files):
            ...
            dst_path = os.path.join(dst, base + ".jpg")
    ...
    def _finish(self, total: int, errors: list) -> None:
        ...
        if errors:
            ...
            messagebox.showwarning(
                ...
                f"Przetworzono {ok} z {total} zdjęć.\n\nBłędy:\n{detail}",
            )
        else:
            messagebox.showinfo("Gotowe!", f"Pomyślnie przetworzono {total} zdjęć.")
